 ...
 */
internal class GalleryScreen : SkScreen(Component.literal("Sighte Addons — UI Gallery")) {
    ...
    // Integers on purpose: every one of these is compared against a mouse coordinate, which arrives
    // as an integer. The drawing converts at the call; the hit test must not, or a click resolves
    // against a rectangle half a pixel from the one on screen.
    ...
    /**
     * Opaque, unlike the settings screen.
     *
     * The gallery is a workbench, not a panel over a game: what it exists to do is judge a colour or a
     * hairline against a known ground, and a blurred dungeon behind it would be a variable in every one
     * of those judgements. Drawn through the dim rather than in [content] so it lands in Minecraft's own
     * pass, underneath the HUD specimens that [extractSpecimens] still draws there.
     */
    override val backdrop: Backdrop get() = Backdrop.NONE

    override val backdropDimTop: Int get() = Tokens.surfaceBase

    override val backdropDimBottom: Int get() = Tokens.surfaceBase
    ...
    /** Where every page starts. A property so both passes agree without passing it between them. */
    private val pageLeft get() = Tokens.SPACE_24.toFloat()

    private val pageTop get() = (Tokens.SPACE_48 + Tokens.SPACE_24).toFloat()

    /**
     * The HUD specimens, drawn through Minecraft's own renderer because the HUD still is.
     *
     * Three pages have them, and they are the three the footer already treats specially. It works out
     * correctly rather than by luck: a specimen lives *inside* a stage rectangle and every annotation
     * this screen draws lives outside one, so the Skija layer landing wholly on top of this one puts
     * nothing over anything. When the HUD draws through [Sk] these fold back into their pages.
     */
    override fun extractExtra(graphics: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        when (page) {
            Page.HUD -> hudSpecimens(graphics, pageLeft, pageTop)
            Page.OVERLAY -> overlaySpecimens(graphics, pageLeft, pageTop)
            Page.SPLITS -> splitsSpecimens(graphics, pageLeft, pageTop)
            else -> {}
        }
    }
    ...
    override fun content() {
        val left = pageLeft
        label("UI GALLERY", left, (Tokens.SPACE_20).toFloat(), Tokens.textPrimary)
        flat(Tokens.palette.name, width - left - w(Tokens.palette.name), (Tokens.SPACE_20).toFloat(), Tokens.textTertiary)
        Sk.fill(left, (Tokens.SPACE_32).toFloat(), width - left * 2, Chrome.HAIRLINE, Tokens.borderSubtle)

        var x = left
        for (entry in Page.entries) {
            val active = entry == page
            flat(entry.label, x, (Tokens.SPACE_32 + 8).toFloat(), if (active) Tokens.textPrimary else Tokens.textTertiary)
            if (active) {
                Sk.fill(x, (Tokens.SPACE_32 + 19).toFloat(), w(entry.label), Chrome.HAIRLINE, Tokens.accent)
            }
            x += w(entry.label) + Tokens.SPACE_16
        }
        Sk.fill(left, (Tokens.SPACE_48 + 12).toFloat(), width - left * 2, Chrome.HAIRLINE, Tokens.borderSubtle)

        val top = pageTop
        when (page) {
            Page.COLOUR -> colourPage(left, top)
            Page.MOTION -> motionPage(left, top)
            Page.DENSITY -> densityPage(left, top)
            Page.HUD -> hudLabels(left, top)
            Page.CONTROLS -> controlsPage(left, top, pointerX, pointerY)
            Page.INPUT -> inputPage(left, top, pointerX, pointerY)
            Page.NAV -> navPage(left, top, pointerX, pointerY)
        ...
        flat(footer, left, (height - Tokens.SPACE_20).toFloat(), Tokens.textTertiary)
    }

    // --- Colour -----------------------------------------------------------------------------

    private fun colourPage(left: Float, top: Float) {
        val p = Tokens.palette
        label("SURFACES", left, top, Tokens.textSecondary)
        ...
        for ((name, colour) in listOf(
            ...
            Sk.fill(x, swatchY, SWATCH.toFloat(), SWATCH_H.toFloat(), colour)
            Sk.border(x, swatchY, (SWATCH).toFloat(), (SWATCH_H).toFloat(), Tokens.borderDefault)
            flat(name, x, swatchY + SWATCH_H + 4, Tokens.textSecondary)
            flat(hex(colour), x, swatchY + SWATCH_H + 14, Tokens.textTertiary)
        ...
        label("TEXT ON EACH SURFACE  ·  MEASURED CONTRAST", left, rampTop, Tokens.textSecondary)
        ...
        for ((surfaceName, surface) in listOf(
            ...
            Sk.fill(left, y, 460f, 22f, surface)
            Sk.border(left, y, (460).toFloat(), (22).toFloat(), Tokens.borderSubtle)
            flat(surfaceName, left + Tokens.SPACE_8, y + 7, Tokens.textTertiary)
            ...
            for ((label, colour) in listOf(
                ...
                flat(note, tx + 40, y + 7, if (passes) Tokens.textTertiary else Tokens.textPrimary)
                tx += 120
            }
            y += 26
        }
        ...
        label("BORDERS  ·  ACCENT", left, borderTop, Tokens.textSecondary)
        var bx = left
        for ((name, colour) in listOf(
            ...
            Sk.fill(bx, borderTop + Tokens.SPACE_12, SWATCH.toFloat(), SWATCH_H.toFloat(), p.surfaceRaised)
            Sk.border(bx, borderTop + Tokens.SPACE_12, (SWATCH).toFloat(), (SWATCH_H).toFloat(), colour)
            flat(name, bx, borderTop + Tokens.SPACE_12 + SWATCH_H + 4, Tokens.textTertiary)
            bx += SWATCH + Tokens.SPACE_8
        }
        Sk.fill(bx, borderTop + Tokens.SPACE_12, SWATCH.toFloat(), SWATCH_H.toFloat(), Tokens.accent)
        flat("accent", bx + 6, borderTop + Tokens.SPACE_12 + 10, Tokens.accentText)
    }

    // --- Motion -----------------------------------------------------------------------------

    private fun motionPage(left: Float, top: Float) {
        label("CURVES  ·  PLOTTED, AND TRAVELLED", left, top, Tokens.textSecondary)
        ...
        for ((name, easing) in curves) {
            Sk.fill(x, plotY, PLOT.toFloat(), PLOT.toFloat(), Tokens.surfaceRaised)
            Sk.border(x, plotY, (PLOT).toFloat(), (PLOT).toFloat(), Tokens.borderDefault)
            ...
            for (step in 0 until PLOT) {
                ...
                Sk.fill(x + step, py, 1f, 1f, Tokens.textPrimary)
            }
            flat(name, x, plotY + PLOT + 4, Tokens.textTertiary)
        ...
        label("LIVE  ·  ${if (Motion.ambientEnabled()) "2000ms LOOP" else "HELD — REDUCE MOTION"}", left, trackTop, Tokens.textSecondary)

        var y = trackTop + Tokens.SPACE_16
        val trackWidth = 320
        val phase = if (Motion.ambientEnabled()) ((Clock.nowMs % LOOP_MS) / LOOP_MS).toFloat() else 0f
        for ((name, easing) in curves) {
            Sk.fill(left, y + 5, (trackWidth).toFloat(), Chrome.HAIRLINE, Tokens.borderSubtle)
            ...
            Sk.fill(dot, y, DOT.toFloat(), DOT.toFloat(), Tokens.textPrimary)
            flat(name, left + trackWidth + Tokens.SPACE_12, y, Tokens.textTertiary)
            y += Tokens.SPACE_16
        }

        // The spring, flipped on a timer so the overshoot is visible without an input.
        val springTop = y + Tokens.SPACE_16
        label("SPRING  ·  ~1.06 OVERSHOOT", left, springTop, Tokens.textSecondary)
        ...
        Sk.fill(left, springY + 5, (trackWidth).toFloat(), Chrome.HAIRLINE, Tokens.borderSubtle)
        ...
        Sk.fill(knobX, springY, DOT.toFloat(), DOT.toFloat(), Tokens.accent)
        flat(
        ...
        flat(ladder, left, springY + Tokens.SPACE_24, Tokens.textTertiary)
    }

    // --- Density ----------------------------------------------------------------------------

    private fun densityPage(left: Float, top: Float) {
        val window = minecraft.window
        label("DEVICE SCALE", left, top, Tokens.textSecondary)
        ...
        for ((name, value) in rows) {
            flat(name, left, y, Tokens.textTertiary)
            flat(value, left + 160, y, Tokens.textPrimary)
            y += Tokens.SPACE_12
        }
        ...
        label("HAIRLINE  ·  DEVICE PIXEL vs GUI PIXEL", left, lineTop, Tokens.textSecondary)
        Sk.fill(left, lineTop + Tokens.SPACE_16, (260).toFloat(), Chrome.HAIRLINE, Tokens.textPrimary)
        flat("1 device px", left + 272, lineTop + Tokens.SPACE_12, Tokens.textTertiary)
        Sk.fill(left, lineTop + Tokens.SPACE_24 + 4, 260f, Chrome.HAIRLINE, Tokens.textPrimary)
        flat("1 gui px", left + 272, lineTop + Tokens.SPACE_24, Tokens.textTertiary)

        val boxTop = lineTop + Tokens.SPACE_48
        Sk.border(left, boxTop, (120).toFloat(), (40).toFloat(), Tokens.borderStrong)
        flat("device-pixel box", left + Tokens.SPACE_8, boxTop + 16, Tokens.textSecondary)

        // Spacing scale.
        val spacingTop = boxTop + Tokens.SPACE_48 + Tokens.SPACE_16
        label("SPACING", left, spacingTop, Tokens.textSecondary)
        var sx = left
        for (step in Tokens.SPACING) {
            Sk.fill(sx, spacingTop + Tokens.SPACE_16, step.toFloat(), 8f, Tokens.textSecondary)
            flat(step.toString(), sx, spacingTop + Tokens.SPACE_16 + 12, Tokens.textTertiary)
        ...
        label("RADII  ·  SQUARE UNTIL THE SHEET LANDS", left, radiusTop, Tokens.textSecondary)
        var rx = left
        for (radius in intArrayOf(Tokens.RADIUS_XS, Tokens.RADIUS_SM, Tokens.RADIUS_MD, Tokens.RADIUS_LG, Tokens.RADIUS_XL)) {
            Sk.fill(rx, radiusTop + Tokens.SPACE_16, 40f, 32f, Tokens.surfaceRaised)
            Sk.border(rx, radiusTop + Tokens.SPACE_16, (40).toFloat(), (32).toFloat(), Tokens.borderDefault)
            flat("r$radius", rx + 4, radiusTop + Tokens.SPACE_16 + 12, Tokens.textTertiary)
     ...
     */
    /** The two cards, through Minecraft's renderer. See [extractExtra]. */
    private fun hudSpecimens(graphics: GuiGraphicsExtractor, left: Float, top: Float) {
        val snapshot = HudPreview.at(previewTime())
        ...
        val stageTop = (top + Tokens.SPACE_16).toInt()
        val originX = left.toInt()
        val stageWidth = HudRoot.WIDTH + Tokens.SPACE_32
        graphics.fill(originX, stageTop, originX + stageWidth, stageTop + STAGE_TALL, Tokens.PREVIEW_STAGE)
        previewHud.draw(graphics, font, snapshot, originX + Tokens.SPACE_16, stageTop + Tokens.SPACE_16)

        // The same card again at double size, for reading the layout and checking that text still fits
        // its box. Note what it does *not* show: under a scaled pose DevicePixels bails to its
        ...
        val zoomX = originX + stageWidth + Tokens.SPACE_24
        val pose = graphics.pose()
        pose.pushMatrix()
        pose.translate(zoomX.toFloat(), stageTop.toFloat())
        pose.scale(2f, 2f)
        graphics.fill(0, 0, stageWidth, STAGE_TALL, Tokens.PREVIEW_STAGE)
    ...
    /**
     * The HUD page's annotations.
     *
     * Drawn twice at two GUI scales side by side, because the two things most likely to be wrong are
     * both scale-dependent: the hairline border and the fixed card width against variable text.
     */
    private fun hudLabels(left: Float, top: Float) {
        val elapsed = previewTime()
        label(
            "SCRIPTED RUN  ·  %.1fs OF %.0fs".format(
                Locale.ROOT, (elapsed % HudPreview.CYCLE_MS) / 1000.0, HudPreview.CYCLE_MS / 1000.0,
            ),
            left, top, Tokens.textSecondary,
        )
        val stageTop = top + Tokens.SPACE_16
        val stageWidth = (HudRoot.WIDTH + Tokens.SPACE_32).toFloat()
        label("AT 2x", left + stageWidth + Tokens.SPACE_24, top, Tokens.textSecondary)
        flat(
            "scrim is the only backdrop — blurBeforeThisStratum would blur the hotbar too",
            left, stageTop + STAGE_TALL + Tokens.SPACE_8, Tokens.textTertiary,
        )
    }
     ...
     */
    private fun controlsPage(left: Float, top: Float, mouseX: Int, mouseY: Int) {
        val word = "save"
        val buttonWidth = Button.width(word) { w(it, Button.SIZE, Type.MEDIUM) }
        val step = buttonWidth + Tokens.SPACE_12
        val right = left + step * 5 + Tokens.SPACE_32

        label("BUTTON", left, top, Tokens.textSecondary)
        val captions = listOf("idle", "hover", "press", "off", "focus")
        for (i in captions.indices) {
            flat(captions[i], left + i * step, top + Tokens.SPACE_12, Tokens.textTertiary)
        }
        ...
        for ((name, variant) in listOf(
            ...
            Button.draw(left, y, (buttonWidth).toFloat(), (Button.HEIGHT).toFloat(), word, variant)
            Button.draw(left + step, y, (buttonWidth).toFloat(), (Button.HEIGHT).toFloat(), word, variant, hover = 1f)
            Button.draw(left + step * 2, y, (buttonWidth).toFloat(), (Button.HEIGHT).toFloat(), word, variant, hover = 1f, press = 1f)
            Button.draw(left + step * 3, y, (buttonWidth).toFloat(), (Button.HEIGHT).toFloat(), word, variant, enabled = false)
            Button.draw(left + step * 4, y, (buttonWidth).toFloat(), (Button.HEIGHT).toFloat(), word, variant, focus = 1f)
            flat(name, left + step * 5, y + Tokens.SPACE_6, Tokens.textTertiary)
        ...
        liveButtonX = (left).toInt()
        liveButtonY = (y + Tokens.SPACE_4).toInt()
        val overLive = hit(mouseX, mouseY, (liveButtonX).toFloat(), (liveButtonY).toFloat(), (buttonWidth).toFloat(), (Button.HEIGHT).toFloat())
        ...
        Button.draw(
            (liveButtonX).toFloat(), (liveButtonY).toFloat(), (buttonWidth).toFloat(), (Button.HEIGHT).toFloat(), word,
            Button.Variant.SECONDARY,
            hover = Controls.hover(anim.of("live.hover"), overLive),
            press = livePress.value,
        )
        flat("live — hover it, hold it", (liveButtonX + step).toFloat(), (liveButtonY + Tokens.SPACE_6).toFloat(), Tokens.textTertiary)

        // --- switches, chips, badges, on the right ---
        label("TOGGLE", right, top, Tokens.textSecondary)
        var ty = top + Tokens.SPACE_16
        for ((name, state) in listOf("off" to 0f, "mid" to 0.5f, "on" to 1f)) {
            Controls.toggle(right, ty, (Controls.TOGGLE_HEIGHT).toFloat(), state, enabled = true)
            flat(name, right + Controls.TOGGLE_WIDTH + Tokens.SPACE_8, ty + Tokens.SPACE_6, Tokens.textTertiary)
            ty += Controls.TOGGLE_HEIGHT + Tokens.SPACE_6
        }
        Controls.toggle(right, ty, (Controls.TOGGLE_HEIGHT).toFloat(), 1f, enabled = false)
        flat("disabled", right + Controls.TOGGLE_WIDTH + Tokens.SPACE_8, ty + Tokens.SPACE_6, Tokens.textTertiary)

        val chipTop = ty + Tokens.SPACE_32
        label("CHIP  ·  BADGE", right, chipTop, Tokens.textSecondary)
        var cx = right
        for ((state, hover) in listOf(0f to 0f, 0f to 1f, 1f to 0f)) {
            Controls.chip(cx, chipTop + Tokens.SPACE_16, (CHIP_H).toFloat(), "puzzle", 14, state, hover)
            cx += Controls.chipWidth("puzzle", 14) { w(it, Tokens.TEXT_11.toFloat()) } + Tokens.SPACE_6
        }
        ...
        Controls.chip(cx, chipTop + Tokens.SPACE_16, (CHIP_H).toFloat(), "no count", -1, 0f, 0f)

        var bx = right
        val badgeY = chipTop + Tokens.SPACE_32 + Tokens.SPACE_6
        Badge.draw(bx, badgeY, "PB", Badge.Style.SOLID)
        bx += Badge.width("PB") { w(it, Badge.SIZE, Type.MEDIUM) } + Tokens.SPACE_6
        Badge.draw(bx, badgeY, "NEW", Badge.Style.OUTLINE)
        bx += Badge.width("NEW") { w(it, Badge.SIZE, Type.MEDIUM) } + Tokens.SPACE_6
        Badge.draw(bx, badgeY, "OFF", Badge.Style.SOLID, enabled = false)
        bx += Badge.width("OFF") { w(it, Badge.SIZE, Type.MEDIUM) } + Tokens.SPACE_6
        Badge.draw(bx, badgeY, "A VERY LONG ONE", Badge.Style.OUTLINE)
     ...
     */
    private fun inputPage(left: Float, top: Float, mouseX: Int, mouseY: Int) {
        val fieldWidth = 150
        val right = left + fieldWidth + 150

        label("TEXT FIELD", left, top, Tokens.textSecondary)
        var y = top + Tokens.SPACE_16

        fun row(caption: String, build: () -> Unit) {
            build()
            flat(caption, left + fieldWidth + Tokens.SPACE_12, y + Tokens.SPACE_6, Tokens.textTertiary)
            y += TextField.HEIGHT + Tokens.SPACE_8
        }

        row("empty · placeholder") {
            TextField.draw(
                left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), TextField.Edit(),
                placeholder = "room name",
            )
        }
        row("hovered") {
            TextField.draw(
                left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), TextField.Edit("Water Board"),
                hover = 1f,
            )
        }
        row("focused · caret on") {
            ...
            edit.placeCaret(5)
            TextField.draw(left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), edit, focus = 1f, caret = 1f)
        }
        row("focused · caret off") {
            val edit = TextField.Edit("Water Board")
            edit.placeCaret(5)
            TextField.draw(left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), edit, focus = 1f, caret = 0f)
        }
        row("selection") {
            val edit = TextField.Edit("Water Board")
            edit.placeCaret(0)
            edit.move(5, extend = true)
            TextField.draw(left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), edit, focus = 1f)
        }
        row("disabled") {
            TextField.draw(
                left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), TextField.Edit("Water Board"),
        ...
        row("live · real blink rate") {
            TextField.draw(
                left, y, (fieldWidth).toFloat(), (TextField.HEIGHT).toFloat(), field,
        ...
        label("MASKED  ·  A SECRET, NOT ECHOED", right, top, Tokens.textSecondary)
        var my = top + Tokens.SPACE_16
        TextField.draw(
            right, my, (fieldWidth + Tokens.SPACE_32).toFloat(), (TextField.HEIGHT).toFloat(), secret,
            placeholder = "paste your key", mask = TextField.Mask.DOTS,
        )
        flat("masked", right + fieldWidth + Tokens.SPACE_48, my + Tokens.SPACE_6, Tokens.textTertiary)
        ...
        TextField.draw(
            right, my, (fieldWidth + Tokens.SPACE_32).toFloat(), (TextField.HEIGHT).toFloat(),
            TextField.Edit(secret.text),
            mask = TextField.Mask.DOTS, revealed = true, focus = 1f, caret = 1f,
        )
        flat("revealed", right + fieldWidth + Tokens.SPACE_48, my + Tokens.SPACE_6, Tokens.textTertiary)
        my += TextField.HEIGHT + Tokens.SPACE_8
        TextField.draw(
            right, my, (fieldWidth + Tokens.SPACE_32).toFloat(), (TextField.HEIGHT).toFloat(), TextField.Edit(),
            placeholder = "paste your key", mask = TextField.Mask.DOTS,
        )
        flat("empty · masked", right + fieldWidth + Tokens.SPACE_48, my + Tokens.SPACE_6, Tokens.textTertiary)

        // --- stepper and slider ---
        val stepperTop = my + Tokens.SPACE_32
        label("STEPPER  ·  A FEW TICKS AT A TIME", right, stepperTop, Tokens.textSecondary)
        val ticks = "138 ticks · 6.90s"
        val stepperWidth = Stepper.width(ticks) { w(it, Stepper.SIZE, Type.MEDIUM) }
        ...
        for ((caption, hovers) in listOf(
            ...
            Stepper.draw(
                right, sy, (stepperWidth).toFloat(), (Stepper.HEIGHT).toFloat(), ticks,
                fraction = 138f / 400f, minusHover = hovers.first, plusHover = hovers.second,
            )
            flat(caption, right + stepperWidth + Tokens.SPACE_12, sy + Tokens.SPACE_4, Tokens.textTertiary)
            sy += Stepper.HEIGHT + Tokens.SPACE_8
        }
        Stepper.draw(right, sy, (stepperWidth).toFloat(), (Stepper.HEIGHT).toFloat(), ticks, 138f / 400f, enabled = false)
        flat("disabled", right + stepperWidth + Tokens.SPACE_12, sy + Tokens.SPACE_4, Tokens.textTertiary)

        val sliderTop = y + Tokens.SPACE_16
        label("SLIDER  ·  A SHAPE, NOT A NUMBER", left, sliderTop, Tokens.textSecondary)
        var ly = sliderTop + Tokens.SPACE_16
        val sliderWidth = 120
        for ((caption, value) in listOf("0" to 0f, "45" to 0.45f, "100" to 1f)) {
            Slider.draw(left, ly, (sliderWidth).toFloat(), (Slider.HEIGHT).toFloat(), value)
            flat(caption, left + sliderWidth + Tokens.SPACE_12, ly + Tokens.SPACE_4, Tokens.textTertiary)
            ly += Slider.HEIGHT + Tokens.SPACE_6
        }
        Slider.draw(left, ly, (sliderWidth).toFloat(), (Slider.HEIGHT).toFloat(), 0.6f, hover = 1f, active = true)
        flat("held", left + sliderWidth + Tokens.SPACE_12, ly + Tokens.SPACE_4, Tokens.textTertiary)
        ly += Slider.HEIGHT + Tokens.SPACE_6
        Slider.draw(left, ly, (sliderWidth).toFloat(), (Slider.HEIGHT).toFloat(), 0.6f, enabled = false)
        flat("disabled", left + sliderWidth + Tokens.SPACE_12, ly + Tokens.SPACE_4, Tokens.textTertiary)
        ly += Slider.HEIGHT + Tokens.SPACE_6
        ...
        val liveFraction = if (hit(mouseX, mouseY, left, ly, (sliderWidth).toFloat(), (Slider.HEIGHT).toFloat())) {
            Slider.fractionAt(left.toInt(), sliderWidth, mouseX)
        } else {
            0.5f
        }
        Slider.draw(left, ly, (sliderWidth).toFloat(), (Slider.HEIGHT).toFloat(), liveFraction, hover = 1f)
        flat(
        ...
        label("PROGRESS  ·  BAR ABOVE TWELVE, SEGMENTS BELOW", right, barTop, Tokens.textSecondary)
        var by = barTop + Tokens.SPACE_16
        for ((caption, value) in listOf("0" to 0f, "1%" to 0.01f, "40%" to 0.4f, "99%" to 0.99f, "100%" to 1f)) {
            ProgressBar.draw(right, by, (120).toFloat(), (ProgressBar.HEIGHT).toFloat(), value)
            flat(caption, right + 120 + Tokens.SPACE_12, by - 2, Tokens.textTertiary)
            by += Tokens.SPACE_12
        }
        Chrome.segments(right, by + 2, 120f, ProgressBar.HEIGHT.toFloat(), 6, 4f, Tokens.accent, Tokens.borderDefault)
        flat("4 of 6, countable", right + 120 + Tokens.SPACE_12, by, Tokens.textTertiary)
    }

    // --- Navigation -------------------------------------------------------------------------

    /** The rail, the segmented switch, the section header, and the overlays that float above them. */
    private fun navPage(left: Float, top: Float, mouseX: Int, mouseY: Int) {
        label("RAIL  ·  LIVE", left, top, Tokens.textSecondary)
        railTop = (top + Tokens.SPACE_16).toInt()
        val names = listOf("hud", "chat", "rooms", "debug")
        val onRail = mouseX >= left && mouseX < left + Nav.WIDTH
        val hovered = Nav.rowAt(railTop, names.size, mouseY).takeIf { onRail } ?: -1
        for (i in names.indices) {
            ...
            Nav.item(
                left, (itemY).toFloat(), (Nav.WIDTH).toFloat(), (Nav.ROW).toFloat(), names[i],
                select.value,
                Controls.hover(anim.of("nav.hover.$i"), hovered == i && i != railItem),
            )
        }
        Nav.divider(left + Nav.WIDTH + Tokens.SPACE_12, (railTop).toFloat(), (Nav.ROW * names.size).toFloat())

        val segLeft = left + Nav.WIDTH + Tokens.SPACE_32
        label("SEGMENTED  ·  LIVE", segLeft, top, Tokens.textSecondary)
        segmentLeft = (segLeft).toInt()
        segmentTop = (top + Tokens.SPACE_16).toInt()
        segmentTravel.springTo(segment.toFloat(), Motion.BASE)
        val segEach = segmentEach()
        val segHover = Segmented.indexAt(SEGMENTS, segEach, segmentLeft, mouseX)
            .takeIf { mouseY in segmentTop until (segmentTop + Segmented.HEIGHT) } ?: -1
        Segmented.draw(
            segmentLeft.toFloat(), segmentTop.toFloat(), Segmented.HEIGHT.toFloat(), segEach.toFloat(),
            SEGMENTS, segment, segmentTravel.value, segHover,
        )
        val segWidth = segEach * SEGMENTS.size
        Segmented.draw(
            segmentLeft.toFloat(), (segmentTop + Segmented.HEIGHT + Tokens.SPACE_8).toFloat(),
            Segmented.HEIGHT.toFloat(), segEach.toFloat(),
            SEGMENTS, 0, 0f, enabled = false,
        )
        flat("disabled", (segmentLeft + segWidth + Tokens.SPACE_12).toFloat(), (segmentTop + Segmented.HEIGHT + Tokens.SPACE_12).toFloat(), Tokens.textTertiary)
        ...
        val headerWidth = 300f
        Chrome.groupLabel(left, (headerTop).toFloat(), headerWidth, "SECTION HEADER", "12 rooms")
        Chrome.groupLabel(left, (headerTop + Tokens.SPACE_16).toFloat(), headerWidth, "NO META")
        // What used to stand here was a tracked-versus-untracked width comparison, because the label
        // style faked its letter-spacing one glyph at a time and the difference was a real number a
        // caller had to lay out against. The bundled face is monospaced, so there is no tracking and
        // nothing to compare — the specimen that replaced it is the one fact still worth stating.
        flat(
            "monospaced — no tracking, and no per-glyph draw to pay for it",
            left, (headerTop + Tokens.SPACE_32).toFloat(), Tokens.textTertiary,
        )
        flat(
            "truncated to 60px: \"${Sk.fit("SECTION HEADER", 60f, LABEL, Type.MEDIUM)}\"",
            left, (headerTop + Tokens.SPACE_32 + Tokens.SPACE_12).toFloat(), Tokens.textTertiary,
        )

        // --- overlays ---
        val overlayTop = headerTop + Tokens.SPACE_48 + Tokens.SPACE_16
        label("POPOVER  ·  TOOLTIP", left, (overlayTop).toFloat(), Tokens.textSecondary)
        Popover.frame(left, (overlayTop + Tokens.SPACE_16).toFloat(), (160).toFloat(), (44).toFloat())
        flat("a popover", left + Popover.PADDING, (overlayTop + Tokens.SPACE_16 + Popover.PADDING).toFloat(), Tokens.textPrimary)
        flat(
            "surfaceOverlay, E2, borderStrong",
            left + Popover.PADDING, (overlayTop + Tokens.SPACE_16 + Popover.PADDING + Tooltip.LINE).toFloat(), Tokens.textTertiary,
        )
        ...
        Tooltip.draw(
            edgeX, overlayTop + Tokens.SPACE_24, width, height,
            listOf("Thunder", "flipped — it would not fit to the right"),
        )
        Sk.fill((edgeX).toFloat(), (overlayTop + Tokens.SPACE_16).toFloat(), Chrome.HAIRLINE, (Tokens.SPACE_24).toFloat(), Tokens.borderStrong)
    ...
    private fun dataPage(left: Float, top: Float, mouseX: Int, mouseY: Int) {
        val tableWidth = 300
        label("TABLE  ·  CLICK A HEADER", left, top, Tokens.textSecondary)

        headerTopY = (top + Tokens.SPACE_16).toInt()
        ...
        for ((index, column) in columns.withIndex()) {
            ...
            val over = mouseX.toFloat() in x0..edge && mouseY in headerTopY until (headerTopY + Table.HEADER)
            Table.headerCell(
                name, x0, edge, (headerTopY).toFloat(), rightAligned,
                sorted = index == 0,
                flip = flip.value,
                hover = Controls.hover(anim.of("data.head.$index"), over && index != 0),
            )
        }
        Table.divider(left, (headerTopY + Tokens.SPACE_12).toFloat(), (tableWidth).toFloat())

        var y = headerTopY + Tokens.SPACE_16
        for ((caption, state) in listOf("idle" to 0, "hovered" to 1, "open" to 2)) {
            Controls.rowHighlight(
                left - Tokens.SPACE_8, (y).toFloat(), (tableWidth + Tokens.SPACE_16).toFloat(), (Table.ROW).toFloat(),
                if (state == 1) 1f else 0f, state == 2,
            )
            val textY = Sk.centreY((y).toFloat(), Table.ROW.toFloat(), BODY)
            flat("Water Board", left, textY, if (state == 2) Tokens.textPrimary else Tokens.textSecondary)
            flat("0:41.2", left + 150 - w("0:41.2"), textY, Tokens.textPrimary)
            flat("0:19.8", left + 220 - w("0:19.8"), textY, Tokens.textPrimary)
            flat("today", left + tableWidth - w("today"), textY, Tokens.textTertiary)
            if (state == 0) {
                Badge.draw(left + 88, (y + (Table.ROW - Badge.HEIGHT) / 2).toFloat(), "PB", Badge.Style.SOLID)
            }
            flat(caption, left + tableWidth + Tokens.SPACE_16, textY, Tokens.textTertiary)
            y += Table.ROW
        }
        ...
        Table.detail(left, (y).toFloat(), (Table.ROW).toFloat(), "clear", "best 0:41.2 · 18 attempts")
        Sparkline.draw(Table.contentX(left) + 140, (y + 2).toFloat(), (72).toFloat(), (Table.ROW - 4).toFloat(), ATTEMPTS, 120)
        y += Table.ROW
        Table.detail(left, (y).toFloat(), (Table.ROW).toFloat(), "floors", "M7 0:41.2 · F7 0:44.8", open = 0.5f)
        y += Table.ROW
        flat("detail at open = 1.0 and 0.5", left + tableWidth + Tokens.SPACE_16, (y - Table.ROW).toFloat(), Tokens.textTertiary)

        // --- empty states, side by side under the table ---
        val emptyTop = y + Tokens.SPACE_16
        label("EMPTY", left, (emptyTop).toFloat(), Tokens.textSecondary)
        val emptyY = emptyTop + Tokens.SPACE_16
        EmptyState.draw(
            left, (emptyY).toFloat(), (200).toFloat(),
            "no history yet", "finish a dungeon room and it lands here", "config/sighteaddons/history.jsonl",
        )
        EmptyState.draw(
            left + 220, (emptyY).toFloat(), (200).toFloat(),
        ...
        EmptyState.draw(
            left + 440, (emptyY).toFloat(), (120).toFloat(),
     ...
     */
    private fun overlaySpecimens(graphics: GuiGraphicsExtractor, left: Float, top: Float) {
        val ms = OverlayPreview.phase(previewTime())
        val l = left.toInt()
        val t = top.toInt()

        stage(graphics, l, t + Tokens.SPACE_12, STAGE_W, STAGE_H) { w, h ->
        ...
        val right = l + STAGE_W + Tokens.SPACE_24
        for ((index, at) in STORM_STEPS.withIndex()) {
            ...
            val cellY = t + Tokens.SPACE_12 + (index / 2) * (STORM_CELL_H + Tokens.SPACE_8)
        ...
        for ((index, popup) in OverlayPreview.POPUPS.withIndex()) {
            val cellX = l + index * (POPUP_CELL_W + Tokens.SPACE_12)
            stage(graphics, cellX, popupTop(top).toInt() + Tokens.SPACE_12, POPUP_CELL_W, POPUP_CELL_H) { w, h ->
    ...
    /**
     * Where the popup row starts.
     *
     * Under both columns and not under the taller-looking one, the same rule the nav page keeps: the
     * scripted half is one stage and the frozen half is two rows of cells, and hard-coding whichever is
     * longer today is how a section lands on top of a specimen the next time one of them grows. Shared
     * by both passes, because they have to agree about it.
     */
    private fun popupTop(top: Float): Float = maxOf(
        top + Tokens.SPACE_12 + STAGE_H + Tokens.SPACE_6 + Tokens.SPACE_24 + Tokens.SPACE_12,
     ...
     * distinguishable from the one before it rather than merely different from a memory of it.
     */
    private fun overlayLabels(left: Float, top: Float) {
        val ms = OverlayPreview.phase(previewTime())
        ...
        timeline(left, stripTop, ms)
        flat(OverlayPreview.caption(ms), left, stripTop + Tokens.SPACE_24, Tokens.textTertiary)

        val right = left + STAGE_W + Tokens.SPACE_24
        label("STORM  ·  FOUR STEPS, FROZEN", right, top, Tokens.textSecondary)
        for ((index, at) in STORM_STEPS.withIndex()) {
            val readout = OverlayPreview.stormAt(at) ?: continue
            val cellX = right + (index % 2) * (STORM_CELL_W + Tokens.SPACE_12)
        ...
        val popupY = popupTop(top)
        label("POPUP  ·  A CLEAR, AND ONE THAT SET A RECORD", left, popupY, Tokens.textSecondary)
        for ((index, popup) in OverlayPreview.POPUPS.withIndex()) {
            val cellX = left + index * (POPUP_CELL_W + Tokens.SPACE_12)
            flat(
                if (popup.pb) {
                    "personal best  ·  chevron, the word, a stronger frame"
                } else {
                    "ordinary  ·  no mark at all"
                },
                cellX, popupY + Tokens.SPACE_12 + POPUP_CELL_H + 2, Tokens.textTertiary,
            )
        }
    }
     ...
     */
    private fun splitsSpecimens(graphics: GuiGraphicsExtractor, left: Float, top: Float) {
        val readout = Splits.sample()
        val l = left.toInt()
        val t = top.toInt()

        val panelH = SplitsHud.measure(readout)
        stage(graphics, l, t + Tokens.SPACE_12, SPLITS_CELL_W, panelH + Tokens.SPACE_16) { _, _ ->
        ...
        val right = l + SPLITS_CELL_W + Tokens.SPACE_24
        stage(graphics, right, t + Tokens.SPACE_12, STORM_CELL_W, STORM_CELL_H) { w, h ->
     ...
     * on screen so a reader is not left wondering whether a missing column is a setting or a defect.
     */
    private fun splitsLabels(left: Float, top: Float) {
        val readout = Splits.sample()

        label("PANEL  ·  F7, MID-RUN, FROZEN", left, top, Tokens.textSecondary)
        val panelH = SplitsHud.measure(readout)
        ...
        label("SPLIT CLOCK  ·  THE RUNNING SPAN ALONE", right, top, Tokens.textSecondary)
        flat(
     ...
     */
    private fun stage(
        graphics: GuiGraphicsExtractor, x: Int, y: Int, w: Int, h: Int, body: (Int, Int) -> Unit,
    ) {
        ...
        // Through `fill` and not `Sk`, like everything else on this pass: the crosshair belongs to the
        // stage, and a stage whose ground is in one layer and whose crosshair is in another is exactly
        // the seam this split is meant to keep out of a single element.
        graphics.fill(w / 2 - CROSSHAIR, h / 2, w / 2 + CROSSHAIR + 1, h / 2 + 1, mark)
        graphics.fill(w / 2, h / 2 - CROSSHAIR, w / 2 + 1, h / 2 + CROSSHAIR + 1, mark)
        body(w, h)
        pose.popMatrix()
    }
     ...
     */
    private fun timeline(left: Float, top: Float, ms: Double) {
        val trackX = left + LANE_LABEL
        val trackW = STAGE_W - LANE_LABEL

        fun span(from: Double, to: Double, y: Float, colour: Int) {
            ...
            Sk.fill(x0.toFloat(), y, (maxOf(x1 - 1, x0 + 1) - x0).toFloat(), LANE_H.toFloat(), colour)
        }

        flat("storm", (left).toFloat(), (top - 2).toFloat(), Tokens.textTertiary)
        Sk.fill((trackX).toFloat(), (top + LANE_H / 2).toFloat(), (trackW).toFloat(), Chrome.HAIRLINE, Tokens.borderSubtle)
        ...
        span(0.0, 3_900.0, (top).toFloat(), Tokens.textTertiary)
        span(3_900.0, 5_900.0, (top).toFloat(), Tokens.textSecondary)
        span(5_900.0, 6_900.0, (top).toFloat(), Tokens.textPrimary)
        span(6_900.0, OverlayPreview.STORM_END_MS, (top).toFloat(), Tokens.accent)

        val popupY = top + LANE_H + Tokens.SPACE_6
        flat("popup", (left).toFloat(), (popupY - 2).toFloat(), Tokens.textTertiary)
        Sk.fill((trackX).toFloat(), (popupY + LANE_H / 2).toFloat(), (trackW).toFloat(), Chrome.HAIRLINE, Tokens.borderSubtle)
        for (popup in OverlayPreview.POPUPS) {
            span(popup.at, popup.at + ClearPopup.LIFE_MS, (popupY).toFloat(), Tokens.textSecondary)
        }

        val playhead = trackX + Math.round(ms / OverlayPreview.CYCLE_MS * trackW).toInt()
        Sk.fill((playhead).toFloat(), (top - 3).toFloat(), Chrome.HAIRLINE, (LANE_H * 2 + Tokens.SPACE_6 + 6).toFloat(), Tokens.accent)
    }

    /** Whether ([mouseX], [mouseY]) is inside a rectangle. */
    private fun hit(mouseX: Int, mouseY: Int, x: Float, y: Float, w: Float, h: Float): Boolean =
        ...
        when (page) {
            ...
            Page.NAV -> {
                ...
                if (mouseY in segmentTop until (segmentTop + Segmented.HEIGHT)) {
                    val index = Segmented.indexAt(SEGMENTS, segmentEach(), segmentLeft, mouseX)
                    if (index >= 0) {
                        segment = index
                        return true
                    }
    ...
    /** Body text, at the size the specimens are annotated in. */
    private fun flat(str: String, x: Float, y: Float, colour: Int) =
        Sk.text(str, x, y, BODY, colour)

    /** How wide [flat] will draw [str], for laying out beside it. */
    private fun w(str: String, size: Float = BODY, family: String = Type.REGULAR): Float =
        Sk.width(str, size, family)

    /**
     * A small uppercase label.
     *
     * It used to be drawn one glyph at a time, because the vanilla bitmap font had no letter-spacing
     * control and the tracking had to be applied by hand — and the old comment here said that seeing
     * that cost was the point of showing it in the gallery. The cost is gone, and not in the way it
     * expected: the bundled face is JetBrains *Mono*, which is already spaced widely enough that a
     * ten-pixel label reads as a label with no tracking at all. One draw, not one per character.
     */
    private fun label(str: String, x: Float, y: Float, colour: Int) =
        Sk.text(str, x, y, LABEL, colour, Type.MEDIUM)

    /** The segmented specimen's segment width, so its draw and its hit test cannot disagree. */
    private fun segmentEach(): Int = Segmented.segmentWidth(SEGMENTS) { w(it, Tokens.TEXT_11.toFloat()) }

    private fun hex(argb: Int): String = String.format(Locale.ROOT, "#%06X", argb and 0xFFFFFF)

    private companion object {
        /** Body text on a specimen page. */
        val BODY = Tokens.TEXT_11.toFloat()

        /** The small uppercase label a section is titled with. */
        ...
        /** How tall the HUD page's card stage is. Was an inline 190 in two places. */
        const val STAGE_TALL = 190
