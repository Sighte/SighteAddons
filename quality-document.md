...
| history and records (`RoomHistory`, `RecordTable`) | B | `RoomHistoryTest` (12 cases; 5 before `recordowner-001`, and the 7 added are the first tests this domain has ever had of *whether a line gets written* rather than of what a line means), `RecordTableTest` | High — the two gates are named predicates with the user's decision and its cost argued at each (`ownClear`, `ownSecretRun`), and the append-only constraint that forbids redefining a metric is stated on `record` itself | Stable, and mutation-checked six times at `9a00325`: dropping the every-secret-was-yours rule fails 1, the vacuous `0 == 0` fails 1, dropping the from-the-start half fails 2, dropping the most-ticks half fails 1, dropping the staleness half fails 1, a null anchor answering `true` fails 1 | **The B was over-generous until 2026-08-15 and is corrected rather than moved.** This file is documented as "the local player's room times" and that claim was false in three ways at once, all of them reported or found on 2026-08-15: a secret run was recorded whatever share of it was yours; a run *started* on a room already half-finished, because the "somebody else's leftovers" guard read this client's own counter, which is 0 for every room until a bar has been read; and a clear was recorded on one second of presence, so arriving as the checkmark landed set a permanent ~1.5 s record. Fixed in `recordowner-001`. **It stays B rather than rising, because neither gate has run in a game** — the predicates are exercised directly and the ten probes measure that they can fail, but the four wiring lines that call them need a live `Minecraft` and the dev client cannot log in. Unchanged gaps: floors are collapsed into one record per room and kind (`records-001`); whole history in memory, ~40 bytes per line. **Not repaired and deliberately not repairable**: the bogus bests already in `history.jsonl` stay records — the user chose that over reinterpreting an append-only file, knowing those rooms will never report a PB again | 2026-08-15 |
